		...
		<div class="p2" id="upload_csv_content" >
			<form id="form_upload_new_product" method="POST" action="/admin/uploadNewProductInventoryCsv" enctype="multipart/form-data">
				...
				<p class="file btn btn-lg btn-theme text-white upload-csv float-start">
					商品を追加
					<input id="input_upload_new_product" type="file" name="file" accept=".csv" v-on:change="AcceptCsvfile($event.target.files, $event.target)"  />
				</p>
			...
			<form id="form_upload_update_stock" method="POST" action="/admin/uploadUpdateStockInventoryCsv" enctype="multipart/form-data">
				...
				<p class="file btn btn-lg btn-theme text-white upload-csv float-start mx-2">
					在庫を更新
					<input id="input_upload_update_stock" type="file" name="file" accept=".csv" v-on:change="AcceptCsvfile($event.target.files, $event.target)"  />
				</p>
			...
			<form ref="form" method="POST" action="<?= $enc->attr( $this->url( $target, $controller, $action, $searchParams, [], $config ) ) ?>">
				...
				<input type="hidden" value="product.status" name="filter[key][3]">
				<input type="hidden" value="==" name="filter[op][3]">
