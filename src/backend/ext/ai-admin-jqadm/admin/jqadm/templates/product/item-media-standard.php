	...
	<div id="item-media-group"
		...
		<div class="group-list" role="tablist" aria-multiselectable="true">
					...
					<div v-bind:id="'item-media-group-data-' + idx" v-bind:class="item['_show'] ? 'show' : 'collapsed'"
						...
						<div class="col-xl-6">
							...
							<div class="form-group media-preview">
								<input id="fileUploadProductImg" ref="preview" class="d-none" type="file" v-bind:name="'media[_idx_][preview]'.replace('_idx_', idx)" accept="image/jpeg, image/png" />
								<input id="fileUploadProductImg2" ref="file" class="fileupload" type="file" tabindex="<?= $this->get( 'tabindex' ) ?>" accept="image/jpeg, image/png"
							...
							<div class="form-group row optional">
								...
								<div class="col-sm-8">
									<input is="flat-pickr" class="form-control listitem-datestart select" type="datetime-local" tabindex="<?= $this->get( 'tabindex' ) ?>"
										v-bind:name="`<?= $enc->js( $this->formparam( ['media', '_idx_', 'product.lists.datestart'] ) ) ?>`.replace('_idx_', idx)"
							...
							<div class="form-group row optional">
								...
								<div class="col-sm-8">
									<input is="flat-pickr" class="form-control listitem-dateend select" type="datetime-local" tabindex="<?= $this->get( 'tabindex' ) ?>"
										v-bind:name="`<?= $enc->js( $this->formparam( ['media', '_idx_', 'product.lists.dateend'] ) ) ?>`.replace('_idx_', idx)"
										placeholder="<?= $enc->attr( $this->translate( 'admin', 'YYYY-MM-DD hh:mm:ss (optional)' ) ) ?>"
										v-bind:disabled="item['product.lists.siteid'] != siteid"
										v-bind:config="Aimeos.flatpickr.datetime"
			...
			<div slot="footer" class="card-tools-more">
				<div id='btnAddProdImg' class="btn btn-primary btn-card-more act-add fa" tabindex="<?= $this->get( 'tabindex' ) ?>"
					title="<?= $enc->attr( $this->translate( 'admin', 'Insert new entry (Ctrl+I)' ) ) ?>"
					v-on:click="$refs.add.click()" >
					<input ref="add" class="d-none" type="file" multiple v-on:change="create($event)" accept="image/jpeg" /> 画像をアップロード
				</div>
			</div>
		</div>
