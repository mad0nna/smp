...
<div id="price" class="item-price tab-pane fade" role="tablist" aria-labelledby="price">
	...
	<div class="card pricecustom d-none">
			...
			<div is="draggable" v-model="items" group="price" handle=".act-move">
				<div v-for="(item, idx) in items" v-bind:key="idx" class="group-item card">
					...
					<div v-bind:id="'item-price-group-data-' + idx" v-bind:class="item['_show'] ? 'show' : 'collapsed'"
						...
						<div class="col-xl-12">
							...
							<div class="form-group row optional col-xl-12">
								<div class="form-group row col-xl-5">
									<label id="txtUnitPrice" class="col-sm-4 form-control-label"><?= $enc->html( $this->translate( 'admin', '単価' ) ) ?></label>
									<div class="col-sm-8">
										<input id=""  class="form-control item-value pe-3" type="number" step="1" tabindex="<?= $this->get( 'tabindex' ) ?>" :min="0" @keypress="isNumber($event)"
							...
							<div class="form-group row mandatory col-xl-5">
								...
								<div class="col-sm-8 pe-4">
									<input id="txtProductPrice" required="required" class="form-control item-value pe-3" type="number" step="1" tabindex="<?= $this->get( 'tabindex' ) ?>" :min="0" @keypress="isNumber($event)" 
							...
							<div class="form-group row optional d-none">
								<label class="col-sm-4 form-control-label "><?= $enc->html( $this->translate( 'admin', 'Granted rebate' ) ) ?></label>
								<div class="col-sm-8">
							...
							<div class="form-group row optional d-none">
								<label class="col-sm-4 form-control-label "><?= $enc->html( $this->translate( 'admin', 'Shipping/item' ) ) ?></label>
								<div class="col-sm-8">
						...
						<div v-show="item['_ext']" class="col-xl-6 secondary d-none">
								...
								<div class="form-group row mandatory">
									...
									<div class="col-sm-8">
										<select is="select-component" required class="form-select listitem-type" tabindex="<?= $enc->attr( $this->get( 'tabindex' ) ) ?>"
											...
											v-bind:name="`<?= $enc->js( $this->formparam( ['price', 'idx', 'product.lists.type'] ) ) ?>`.replace('idx', idx)"
											v-bind:text="`<?= $enc->js( $this->translate( 'admin', 'Please select' ) ) ?>`"
							...
							<?php else : ?>
								<input class="listitem-type" type="hidden"
									v-bind:name="`<?= $enc->js( $this->formparam( array( 'price', 'idx', 'product.lists.type' ) ) ) ?>`.replace('idx', idx)"
							...
							<div class="form-group row optional">
								...
								<div class="col-sm-8">
									<input is="flat-pickr" class="form-control listitem-datestart select" type="datetime-local" tabindex="<?= $this->get( 'tabindex' ) ?>"
										v-bind:name="`<?= $enc->js( $this->formparam( array( 'price', 'idx', 'product.lists.datestart' ) ) ) ?>`.replace('idx', idx)"
							...
							<div class="form-group row optional">
								...
								<div class="col-sm-8">
									<input is="flat-pickr" class="form-control listitem-dateend select" type="datetime-local" tabindex="<?= $this->get( 'tabindex' ) ?>"
										v-bind:name="`<?= $enc->js( $this->formparam( array( 'price', 'idx', 'product.lists.dateend' ) ) ) ?>`.replace('idx', idx)"
										placeholder="<?= $enc->attr( $this->translate( 'admin', 'YYYY-MM-DD hh:mm:ss (optional)' ) ) ?>"
										v-bind:disabled="item['product.lists.siteid'] != siteid"
										v-bind:config="Aimeos.flatpickr.datetime"
