      ...
      <div>
        {
          widgetState.filter(widget=>widget.label !=='').map((widget, index) => {
            return (<div key={index} className="h-10 w-full py-6 text-gray-500 font-sans text-md mb-2 mt-2 pl-2 flex content-center justify-around font-bold text-center">
              <span className=" col-span-1 w-6/12 pl-10 mx-auto"> {widget.label} </span>
              <span className="col-span-1 w-6/12 mx-auto"> <input type="checkbox" className="bg-primary-200 border-primary-200 w-6 h-6" onChange={(e)=>onChangeHandler(e,index)} value={index} defaultChecked={widget.state}/></span>
            </div>)
          })
        }
      </div>
